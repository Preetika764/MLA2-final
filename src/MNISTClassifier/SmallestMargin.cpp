...
double testingProcessBagging(MNISTDatasetReader dataset, ANN *model1, ANN *model2, ANN *model3, ANN *model4, ANN *model5){
    ...
    for (int i = 0; i < numTestBatches; ++i) {
        ...
        pair<Vector, vector<int> > testBatch = dataset.getTestBatch();
        vector<int> outputs1 = (*model1).predictOutputs(testBatch.first);
        vector<int> outputs2 = (*model2).predictOutputs(testBatch.first);
        vector<int> outputs3 = (*model3).predictOutputs(testBatch.first);
        vector<int> outputs4 = (*model4).predictOutputs(testBatch.first);
        vector<int> outputs5 = (*model5).predictOutputs(testBatch.first);
        vector<int> outputs;
...
vector<double> QBC1(vector<int> * prediction1, vector<int> * prediction2,vector<int> * prediction3, vector<int> * prediction4, vector<int> * prediction5)
{
...
int main(int argc, char** argv) {
    ...
    cout << "Using smallest margin for sampling\n";
    ...
    for (int i = 0; i < X/600; i++)
    {
        ...
        for (int i = 0; i < numTrainBatches; ++i) {
            ...
            for (int j = 0; j < tempSmallMargin.size(); j++)
            {
                // softmax.push_back(tempSoftmax[j]);
                 smallMargin.push_back(tempSmallMargin[j]);
            }
        }
        ...
        specialSort(&unlabeledPool, &unlabeledPoolLabels, &smallMargin);
...
int main(int argc, char** argv) {
    ...
    for (int i = 0; i < X/600; i++)
    {
        ...
        vector<double> voteEntropy = QBC1(&predictions[0], &predictions[1], &predictions[2], &predictions[3],&predictions[4]);        
